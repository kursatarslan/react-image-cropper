...
var Cropper = function (_Component) {
  ...
  _createClass(Cropper, [{
    ...
    value: function save() {
      var img = findDOMNode(this.img);
      var canvas = document.createElement('canvas');
      var _values$original2 = this.values().original,
          x = _values$original2.x,
          y = _values$original2.y,
          width = _values$original2.width,
          height = _values$original2.height;
      ...
      canvas.height = height;
      canvas.getContext('2d').drawImage(img, x, y, width, height, 0, 0, width, height);
      return { canvas: canvas.toDataURL(), display: {
          width: frameWidth,
          height: frameHeight,
          x: originX,
          y: originY,
